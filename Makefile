...
publish-docker-cli:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_package_scanner_cli:$(DF_IMG_TAG)

.PHONY: docker-cli
docker-cli:
	docker build --build-arg MAKE_CMD="cli" -t $(IMAGE_REPOSITORY)/khulnasoft_package_scanner_cli:$(DF_IMG_TAG) .

.PHONY: docker-cli-multi-arch-push
docker-cli-multi-arch-push: buildx
	docker buildx build --build-arg MAKE_CMD="cli" --platform linux/arm64,linux/amd64 --tag $(IMAGE_REPOSITORY)/khulnasoft_package_scanner_cli:$(DF_IMG_TAG) . --push

.PHONY: docker-multi-arch
docker-multi-arch:
	docker buildx build --platform linux/arm64,linux/amd64 --tag $(IMAGE_REPOSITORY)/khulnasoft_package_scanner:$(DF_IMG_TAG) .
...
.PHONY: update-sdk
update-sdk:
	go get -u -v github.com/khulnasoft-lab/golang_sdk/client@latest
	go get -u -v github.com/khulnasoft-lab/golang_sdk/utils@latest
