...
import (
	...
	dschttp "github.com/khulnasoft-lab/golang_sdk/utils/http"
	"github.com/khulnasoft-lab/golang_sdk/utils/tasks"
	"github.com/khulnasoft-lab/package-scanner/jobs"
	"github.com/khulnasoft-lab/package-scanner/output"
...
func RunGrpcServer(pluginName string, config utils.Config) error {
	...
	config.KhulnasoftKey = os.Getenv("KHULNASOFT_KEY")

	if dschttp.IsConsoleAgent(config.ConsoleURL) && strings.Trim(config.KhulnasoftKey, "\"") == "" {
		...
		for {
			var err error
			if config.KhulnasoftKey, err = dschttp.GetConsoleApiToken(internalURL, internalPort); err != nil {
				log.Error(err)
...
func processSbomGeneration(configInterface interface{}) interface{} {
	...
	// Send sbom to Khulnasoft Management Console for Vulnerability Scan
